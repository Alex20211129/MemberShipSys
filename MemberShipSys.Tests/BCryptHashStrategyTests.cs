...
{
    public class BCryptHashStrategyTests
    {
        [Fact]
        public void Hash_ReturnsExpectedHash()
        {
            // Arrange：準備測試需要的東西
            var bcrypt = new BCryptHashStrategy();
            // Act：執行你要測的那個動作
            string HashedResult = bcrypt.Hash("MyPassword123");
            bool VerifiedResult = bcrypt.Verify("MyPassword123", HashedResult);
        ...
        public void Hash_ReturnsExpectedWrongPassword()
        {
            ...
            // Act：執行你要測的那個動作
            string HashedResult = bcrypt.Hash("MyPassword123");
            bool VerifiedResult = bcrypt.Verify("WrongPassword", HashedResult);
            // Assert：斷言結果符合預期，不符合的話這個測試就會顯示失敗
            Assert.NotNull(HashedResult);
